document.addEventListener("DOMContentLoaded", () => {
    ...
    const printButton = document.getElementById("print-results"); // Get print button
    ...
    function displayPage1(data) {
        ...
        // Calculate total initial fee from individual resort fees
    ...
    function displayResortComparisonPage(data, resortIndex, pageId) {
        ...
        // Check if resort exists and has a name AND a fee
        if (!resort || !resort.name || !resort.fee) {
        ...
        // Use the specific fee for this resort
    ...
    calculatorForm.addEventListener('submit', (event) => {
        ...
        const data = {
            ...
            // Store resorts with their specific fees
            resorts: [
                { 
        ...
        // Ensure Resort 2 has a fee if it has a name, otherwise clear it
        if (data.resorts[1] && !data.resorts[1].fee) {
            data.resorts[1].fee = null; // Or 0, depending on desired behavior if fee is omitted
        }

        console.log("Captured Data:", data); // Log captured data for debugging

        // Perform calculations and display results
        displayPage1(data);
        displayResortComparisonPage(data, 0, 'page2'); // Display Resort 1
        displayResortComparisonPage(data, 1, 'page3'); // Display Resort 2 (will hide if no name/fee)
    ...
    console.log("Iframe Calculator App Initialized (v2)");
});
